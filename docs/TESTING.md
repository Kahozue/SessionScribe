...
6. **本機 AI 整理事件**：檢視頁右欄產生草稿後，按魔杖鈕做 AI 整理。若本機 Apple Intelligence 未開或不支援該語言，按鈕應停用並顯示原因（機械草稿仍可用）。可用時整理後事件的型別／主題／摘要／待辦被補齊、仍標 needs_review，且 content 與來源段落不變。事件區塊標題可點 chevron 摺疊。
