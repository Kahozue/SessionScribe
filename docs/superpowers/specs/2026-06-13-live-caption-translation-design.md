...
- 翻譯做成設定開關，預設關；目標語言設定可選，預設中文。
- 辨識語言可選（中/英/日，CaptionLanguage），取代寫死 zh-TW，同時決定 SpeechTranscriber locale
  與翻譯來源。英翻中＝英文辨識＋中文目標；日翻中同理。來源＝目標時不翻。
- 翻譯策略固定 `TranslationSession.Strategy.lowLatency`，以 `#available(macOS 26.4)` 守衛，
  不動 deployment target（spike 已證實可行，較不侵入；低於 26.4 翻譯停用、僅原文）。
- 譯文只即時顯示、不持久化（不動 JSONL / DATA_FORMATS schema）。
- 字幕浮層用獨立字級 `captionFontSizeKey`，與主視窗 `fontSize` 脫鉤。

設計變更紀錄：原設計假設來源固定 zh-TW、目標預設英文。使用者澄清主用途為英翻中、日翻中，來源即英/日，
故辨識語言必須開放可選、目標預設改中文。sandbox spike 已通過（沙盒零網路下 lowLatency 模型可下載＋
on-device 推論），Phase 3 放行。
...
## 階段三（M11）即時翻譯：設定開關，預設關（已實作）

前置 spike 已通過：app 為 App Sandbox 無 network entitlement、OS 強制零網路。臨時 app 內 spike
（direct-init + lowLatency + prepareTranslation + translate）實機驗證「沙盒零網路下翻譯模型可下載＋
on-device 推論」成功（zh→en 測試句翻出正確英文），spike 已移除。改採 `#available(macOS 26.4)` 守衛
而非升 deployment target。

辨識語言：`CaptionLanguage`（zh-TW/en/ja）設定，取代 newSession 寫死的 zh-TW，同時為翻譯來源。
