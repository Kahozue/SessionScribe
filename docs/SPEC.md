...
核心場景是論文口試。長期場景包含課堂講座、會議記錄、訪談、研究討論、讀書會、專案會議、客戶訪談、研討會、個人語音筆記。資料模型、UI 文案與功能設計不得綁死在論文口試，論文口試只是預設模板之一。

## 二、敲定階段的修訂決議

以下十項是原始需求之外新增或修改的決議，全部已確認：
...
9. **文件歸宿**：本 SPEC.md 是正式規格，原始需求已全數整併於此。
...
- 開發與驗證以 Apple Silicon Mac、最新版 Xcode 為準。
...
- 主引擎人選以目標機器實測 zh-TW 可用性的結果決定（已驗證支援）。
...
`speaker` 是可 null 欄位。預設 OpenAI STT model `gpt-4o-mini-transcribe` 回傳純逐字文字，不提供 speaker label；若使用者手動改用 `gpt-4o-transcribe-diarize`，`diarized_json` 回應會保存供應商回傳的 speaker label。未提供 speaker 的引擎寫入 null。volatile 結果只存在記憶體；debug 模式可另寫 `volatile_debug.jsonl`，永不混入 live_segments。
...
v0.2 已增加：模板化 `structured_notes.md`（論文口試格式照內建範本，其餘模板用通用版）、`events.json`、`events.csv`、單一 `<session_id>.m4a`。events 可來自既有 `events.json`，或匯出時由 markers 即時生成草稿。
...
1. 第一版不做本機 speaker diarization；雲端 OpenAI STT 若回傳 speaker label，僅保存到逐字稿段落。
...
5. **隱私強制與提示**：唯一的 `URLSession` 只在 `SSCore/Cloud` 層；首次開總開關跳警告（說明選定文字會送往供應商、音訊永遠不送、產物標需複查）；跑雲端操作時把該 session 的 `privacyMode` 記為 `text_cloud_assist`；非 Local Only 時主錄音畫面與檢視頁標頭顯示狀態標。只送選定文字（摘要送 finalized 逐字稿、整理送事件 content 或逐字稿片段），絕不送音訊或原始 chunk。（本項描述的警告文案與「音訊永遠不送」前提僅適用規格 1.3 的文字整理；規格 1.4 起警告文案已改寫為依各功能設定、轉錄稿選雲端會上傳音訊，見第十七節第7項。）
...
延續規格 1.3，把雲端選項從「事件整理＋整份摘要」擴張為五項功能各自獨立選本地或雲端，並為已轉錄 session 補上重新轉錄入口。
...
3. **文字類與語音類兩個供應商槽**：`CloudLLMSettings` 分 `textProviderID`（摘要/結構化事件/字幕翻譯共用）與 `audioProviderID`（離線轉錄稿/即時 ASR 共用），各自選一個 active 供應商。語音類供應商選單只列 `CloudProviderFormat.supportsSTT == true` 者（OpenAI 相容、Gemini；Anthropic 無 STT API，不列入）。語音槽新增供應商使用語音專用樣板，OpenAI 預設 model 為 `gpt-4o-mini-transcribe`。
4. **雲端離線轉錄稿（音訊上傳）**：離線轉錄稿選雲端、語音類供應商與 key 齊備時，`AssistResolver.sttClient` 回傳 `OpenAISTTClient`（`POST /audio/transcriptions`，multipart；預設 `gpt-4o-mini-transcribe` 使用 `response_format=json`，通常只回全文；`gpt-4o-transcribe-diarize` 使用 `response_format=diarized_json` 與 `chunking_strategy=auto`，可保存 speaker label；Whisper 或其他相容舊模型沿用 `verbose_json` 取 segment 時間）或 `GeminiSTTClient`（`generateContent` inline audio，整段一句、無 segment 時間）。`CloudTranscriber` 重用 `AudioExporter` 把整段 session 音訊匯出為單一 `.m4a` 上傳，回傳結果套用名詞表後以 `TranscriptSegment` 落盤（`engine: "cloud"`；無 segment 時間者以整段音訊總長補單段 end time）。成功後該 session 的 `privacyMode` 標為 `audioCloudASR`（`audio_cloud_asr`）；若文字類雲端也曾成功，標為 `textAndAudioCloud`（`text_and_audio_cloud`）。任一條件不齊備（總開關關、該功能非雲端、供應商非 STT 格式、key 缺）即回 nil，路由層退回本地離線轉寫。
...
- Gemini STT 與預設 `gpt-4o-mini-transcribe` 不回傳 segment 級時間時，整段音訊產生一筆逐字稿並以音訊總長作為結束時間；OpenAI 相容端點若回傳 `verbose_json` 或 `diarized_json` 的 `segments`，則按其相對時間對齊媒體時間並保留可用的 speaker label。
